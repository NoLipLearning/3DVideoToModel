...
**Actually achieved:** all four exist. They're verified to the extent this sandbox allows, and each note says exactly how far that is.
- **Learned features (`backends/hloc_backend.py`), verified for real:** DISK keypoints + LightGlue matches are written into COLMAP's database. Two-view verification, the incremental mapper, undistortion and export are unchanged. On 30 fixture frames: **30/30 registered, 0.99px reprojection error, track length 6.7**, ~2 min on CPU. It uses kornia's models (the optional `learned` extra: `uv sync --extra learned`) instead of the `hloc` package, which isn't on PyPI. **DISK is the verified default.** ALIKED is selectable but unverified here, because its weights host returns 403 in this sandbox. SuperPoint is deliberately *not* offered: its weights are non-commercial and kornia has no SuperPoint extractor. Pairing is a cyclic sequential window, so orbits close without a vocabulary tree. With a GPU (CUDA/MPS) and ≤80 frames it is exhaustive, as with SIFT; on CPU LightGlue costs ~1s per pair. `sfm.feature_backend=disk|aliked` selects it, and `sfm.learned_fallback` (on by default) retries with DISK automatically when both SIFT passes miss the registration bar and kornia is installed. **That fallback is verified:** with SIFT's threshold set so neither pass finds any features (0 images registered), attempt 3 with DISK registered ≥90% of the fixture.
- **Scene tiling (`dense/tiling.py`):** the doc's "tile above 30 m" can't be applied before Phase 4 knows the scale. So the voxel a capture deserves is taken as a **ratio**: median sparse-point depth × `tsdf_voxel_size_m / nominal_capture_distance_m` (object 4mm at 0.5m; scene 2cm at 5m). The scene is tiled only when one 400³ volume can't hold that detail. Tiles overlap by a margin, each keeps only its core (no duplicated seams), and each is integrated only from frames whose depth maps reach it. Only one volume is alive at a time, so peak RAM matches the single-volume preflight estimate. `scene_tile_threshold_m` is gone; `nominal_capture_distance_m` and `max_tiles` replace it. Objects never tile, and their path is the unchanged pre-M9 code. Verified on the fixture by forcing ~2 tiles per axis: same extent as the single volume, median distance to the single-volume surface under 1% of the object size, no duplicate points.
- **Moving-object suppression (`dense/transients.py`):** for each dense point, every aligned depth map votes *support* (it saw this surface), *free-space violation* (it saw past it), or nothing (occluded). A point seen through more often than seen, by ≥2 frames, is removed. Occlusion is explicitly not a vote, which is what stops a plain depth-variance threshold from eating real geometry. On in the scene preset, off for objects. Verified on synthetic geometry (a box present in 4 of 20 frames is removed and the wall behind it kept); not verified on real street footage.
- **OpenMVS (`dense/openmvs.py`), orchestration only:** InterfaceCOLMAP on the undistorted workspace Phase 2a already writes, then `DensifyPointCloud --cuda-device -2` (CPU). It is opt-in only (`dense.backend=openmvs`), and "auto" never picks it, even with CUDA, because monocular depth is the backend that copes with textureless surfaces. OpenMVS isn't installed here, so the tests run it against stand-in binaries: the command lines, the CPU flag, output pickup, and failure logs are verified; OpenMVS's own output is not.
